...
class latte_contactme_widget extends WP_Widget {

	function __construct() {
		parent::__construct(
			'latte_contactme_widget',
			__('Latte - Contact Me Widget', 'latte'),
			array('description' => __('Contact Me widget for Latte theme\'s Contact Me section.', 'latte'))
		);
	}
	...
	function form($instance) {
		...
		<p>
			...
			<input id="<?php echo esc_attr($this->get_field_id('newwindow')); ?>" name="<?php echo esc_attr($this->get_field_name('newwindow')); ?>" type="checkbox" value="1" <?php if (isset($instance['newwindow'])): if (1 == $instance['newwindow']) {
					...
					<div data-sr="ease-in-out wait 0.25s" class="col-md-4 col-sm-6 text-center service-box">
						...
						<?php if (!empty($instance['icon']) || !empty($instance['image'])): ?>
							<div class="service-icon">
								<a <?php if (isset($instance['newwindow'])): if (1 == $instance['newwindow']) {
				echo 'target="_blank"';
			}
		endif?> <?php if (!empty($instance['link'])): echo 'href="' . esc_url($instance['link']) . '"';endif;?>>
								<?php if (isset($instance['type']) && $instance['type'] == 0): ?>
...
add_action('widgets_init', create_function('', 'return register_widget("latte_contactme_widget");'));
?>